...
ativos_imperio = {
    ...
    "FINTECH_GLOBAL_ARBITRAGE": ["exchan", "neteller", "pay", "eth", "exodus", "faucet", "miner", "blockchain", "wallet", "btc", "coin", "doge", "dollar", "skrill", "trust", "metamask", "binance", "stripe", "kucoin", "okx", "phantom", "ronin", "advcash", "perfectmoney", "wise", "revolut", "kraken", "mexc", "gateio", "trezor", "ledger", "venmo", "cashapp", "zelle", "braintree", "adyen", "picpay", "mercadopago", "pix", "usdt", "solana", "polygon", "ltc", "defi", "dex", "bingx", "phemex", "coinex", "changelly", "changenow", "paxum", "mercury", "revolutbusiness", "wirex", "blackcatcard", "paxful", "remitly", "westernunion", "monzo", "tether", "clabe"],
    ...
    "GREY_MARKET_POWER": ["g2a", "eneba", "eldorado", "playerauctions", "kinguin", "cdkeys", "zeerk", "igvault", "z2u", "epicnpc", "ogusers", "crackedio", "nulledio", "breachforums", "ownedcore", "nulled", "cracked", "leaked", "breached", "combo"],
    
    "IA_BIO_WEALTH_AGENTS": ["chatgpt", "openai", "claude", "elevenlabs", "runway", "midjourney", "anthropic", "replicate", "huggingface", "leonardo", "synthesia", "descript", "copyai", "pika", "scaleai", "gemini", "jasper", "ollama", "anythingllm", "langchain", "copilot", "perplexity", "stablediffusion", "mistral", "sora", "characterai", "perceive", "neuroama", "biorender", "pubmed", "dnaweekly", "groq", "llama", "deepseek", "flux"],
    ...
    "RISK_PRO_BETTING": ["giftcard", "option", "captcha", "prize", "reward", "buy", "shop", "spin", "vip", "stake", "bcgame", "gamdom", "csgoroll", "aviator", "blaze", "roleta", "cassino", "crash", "slots", "aposta", "lootbox", "bet", "pinnacle", "1xbet", "rollbit"],
    ...
    "NET_DA_RUA_NDR": ["starlink", "ipfs", "ubiquiti", "unifi", "helium", "zerotier", "gl.inet", "mikrotik", "openwrt"],
    
    "CYBER_OSINT_AUDIT": ["maltego", "spiderfoot", "intelx", "hunterio", "builtwith", "breachdirectory", "snusbase", "fullcontact"],
...
FLUSH_LIMIT = 50000  # Reduzi levemente o flush para acomodar o buffer mais complexo
...
def processo_blindado(arquivo):
    ...
    if tamanho_arquivo == 0:
        return f"[-] Ignorado: {arquivo} (Vazio)"

    # Inicialização do Buffer Aninhado: {Categoria: {Keyword: []}}
    ...
    try:
                    ...
                    for categoria, palavras in ativos_imperio.items():
                        for p in palavras:
                            if p in linha_lower:
                                ...
                                hits_totais += 1
                                encontrado = True
                                break # Achou a keyword, próxima linha
                        if encontrado: break
        ...
        descarregar_pro_disco(buffer)
        return f"[+] REFINADO: {arquivo} processado por keyword."
    except Exception as e:
        return f"[!] FALHA em {arquivo}: {e}"

def apogeu_merge():
    """Unifica os parciais e aplica a purga de duplicatas (CARRASCO) via Set Python."""
    print("[*] Consolidando Ativos de Luxo (Purgando Duplicatas)...")
    for categoria, palavras in ativos_imperio.items():
        ...
        for kw in palavras:
            ...
            # Execução de Elite: Lê parciais, remove duplicatas em memória e salva o TOTAL
            try:
                with open(arquivo_parcial, 'r', encoding='utf-8', errors='ignore') as f:
                    linhas_unicas = set(f.readlines())
                
                with open(arquivo_final, 'w', encoding='utf-8') as f:
                    f.writelines(sorted(linhas_unicas))
                
                os.remove(arquivo_parcial) # Purga do rastro parcial
            except Exception as e:
                print(f"[!] Erro ao consolidar {kw}: {e}")
...
def executar_soberania():
    ...
    print(f"[*] MOTOR ATIVADO: {nucleos} núcleos em Ponto de Ignição.")
    with Pool(processes=nucleos) as pool:
        for resultado in pool.imap_unordered(processo_blindado, arquivos):
            print(resultado)

    apogeu_merge()
    print("\n[+] SUCESSO: Refinaria operando em escala total. Lucro consolidado.")
